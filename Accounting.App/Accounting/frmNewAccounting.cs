...
namespace Accounting.App
{
    public partial class frmNewAccounting : Form
    {
        ...
        private void frmNewAccounting_Load(object sender, EventArgs e)
        {
            ...
            if (AccountID!=0)
            {
                var account = db.AccountingRipository.GetById(AccountID);
            ...
            if (BaseValidator.IsFormValid(this.components)) 
            {
                if (rbPay.Checked || rbRecive.Checked)
                {
                    ...
                    DataLayer.Accounting accounting = new DataLayer.Accounting()
                    {
                        ...
                        CustomerID = db.CustomerRepository.CustomerIdByName(txtName.Text),
                    ...
                    if (AccountID==0)
                    {
                        db.AccountingRipository.Insert(accounting);
                    }
                    else
                    {
                        using (UnitOfWork db2=new UnitOfWork())
                        {
                            ...
                            db.AccountingRipository.Update(accounting);
                        }
