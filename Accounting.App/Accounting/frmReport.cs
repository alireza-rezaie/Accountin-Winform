        ...
        void Filter()
        {
            using (UnitOfWork db = new UnitOfWork())
            {
                ...
                if ((int)cbCustomer.SelectedValue != 0)
                {
                    ...
                    result.AddRange(db.AccountingRipository.Get(a => a.TypeID == TypeID && a.CustomerID == customerId));
                }
                else
                {
                    result.AddRange(db.AccountingRipository.Get(a => a.TypeID == TypeID));
                }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dgReport.CurrentRow != null)
            {
                ...
                if (RtlMessageBox.Show("ایا از حذف مطمئن هستید؟", "هشدار", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    using (UnitOfWork db = new UnitOfWork())
                    {
                        db.AccountingRipository.Delete(id);
                        db.Save();
                        Filter();
                    }
                }
